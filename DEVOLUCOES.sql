...
-- DEVOLUCOES FATURAMENTO
   ...
   ,SUM((PP.PEDF_VLR_TOT + NVL(PP.PEDF_VLR_IPI,0)
    ...
    WHERE P.PEDF_SITUACAO                            = 0
      AND NFE.PEDF_NFE_DTA_CAD                       >= TO_DATE(&dataInicial || ' 00:00:00','DD/MM/YYYY HH24:MI:SS')
      AND NFE.PEDF_NFE_DTA_CAD                       <= TO_DATE(&dataFinal || ' 23:59:59','DD/MM/YYYY HH24:MI:SS')
   ...
   GROUP BY
    ...
    SELECT DISTINCT
     ...
     ,SUM(EE.ENTR_PRECO+ EI.ENTR_VLR_IMP + EE.ENTR_DESPESA)AS DEVOLUCOES
    FROM PEDIDO_FAT P
    INNER JOIN OPERACAO_FAT O        ON O.OPER_ID                  = P.PEDF_OPER_ID
    INNER JOIN LIQUIDACAO LQ         ON LQ.LIQU_EMP_ID             = P.PEDF_LIQU_EMP_ID
                                    AND LQ.LIQU_ID                 = P.PEDF_LIQU_ID
    ...
    INNER JOIN ENTRADA_ALM_EI  EI    ON EI.ENTR_ENTR_E_ENTR_EMP_ID = EE.ENTR_ENTR_EMP_ID
    ...
    WHERE P.PEDF_SITUACAO                         = 0
       ...
       AND EA.ENTR_DTA_LANC                       >= TO_DATE(&dataInicial || ' 00:00:00','DD/MM/YYYY HH24:MI:SS')
       AND EA.ENTR_DTA_LANC                       <= TO_DATE(&dataFinal || ' 23:59:59','DD/MM/YYYY HH24:MI:SS')
